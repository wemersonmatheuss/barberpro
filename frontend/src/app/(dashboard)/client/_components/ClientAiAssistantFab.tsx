...
type Topic = 'proximos' | 'historico' | 'agendar' | 'perfil'

type Phase = 'pick' | 'confirm' | 'loading' | 'done'

const TOPIC_OPTIONS: { id: Topic; label: string; hint: string }[] = [
  { id: 'proximos', label: 'Resumo dos meus próximos cortes', hint: 'Datas, serviços e lembretes (demo)' },
  { id: 'historico', label: 'Resumo do meu histórico', hint: 'Cortes passados e status' },
  { id: 'agendar', label: 'Dicas para agendar', hint: 'Escolha de horário e confirmação' },
  { id: 'perfil', label: 'Minha conta e dados', hint: 'Perfil, contato e privacidade' },
]

function buildSummary(topic: Topic, displayName: string): string {
  const first = displayName.split(' ')[0] || 'Cliente'

  switch (topic) {
    case 'proximos':
      return `Olá, ${first} — seus próximos cortes (demo)\n\n• Você pode ver o próximo horário em Início ou em Novo agendamento.\n• Confirme presença com antecedência; em caso de imprevisto use adiar ou cancelar conforme as regras da barbearia.\n• Chegue alguns minutos antes para não perder o slot.\n\nSugestão: salve o WhatsApp da unidade nos favoritos para avisos rápidos.`
    case 'historico':
      return `Olá, ${first} — seu histórico (demo)\n\n• Em Meus cortes você vê cortes anteriores, status (concluído, cancelado, adiado) e valores.\n• Use o histórico para repetir o mesmo serviço ou profissional na próxima visita.\n\nSugestão: avalie o atendimento quando a barbearia disponibilizar avaliações na app.`
    case 'agendar':
      return `Olá, ${first} — agendar com tranquilidade (demo)\n\n• Escolha serviço, profissional e horário com calma; horários cheios aparecem indisponíveis.\n• Revise data e hora antes de confirmar.\n• Depois do agendamento, anote ou adicione ao calendário do celular.\n\nSugestão: prefira horários fora de pico se você tiver flexibilidade.`
    case 'perfil':
      return `Olá, ${first} — sua conta (demo)\n\n• Em Meu perfil você atualiza nome, contato, foto e dados usados na reserva.\n• Mantenha WhatsApp e e-mail corretos para receber lembretes.\n\nSugestão: revise seus dados sempre que trocar de número ou e-mail.`
    default:
      return ''
  }
}

export function ClientAiAssistantFab() {
  const { profile } = useClientProfile()
  const displayName =
    [profile.firstName?.trim(), profile.lastName?.trim()].filter(Boolean).join(' ') || 'Cliente'
      ...
      <div className="pointer-events-none fixed bottom-5 right-5 z-[200] flex flex-col items-end gap-3 md:bottom-8 md:right-8">
        {open ? (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="client-ai-title"
          >
            <header className="flex items-center justify-between gap-2 border-b border-zinc-700/45 bg-gradient-to-r from-violet-600/25 to-cyan-600/20 px-4 py-3">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <div className="min-w-0">
                  <p id="client-ai-title" className="truncate text-sm font-bold text-primary">
                    Assistente (demo)
                  </p>
                  <p className="truncate text-xs text-secondary">Resumos para sua experiência</p>
                </div>
              </div>
              <button
                type="button"
              ...
              {phase === 'pick' && (
                <>
                  <p className="text-sm leading-relaxed text-secondary">
                    ...
                    para eu montar um <strong className="text-primary">resumo rápido</strong> (simulado) para você.
                  </p>
              ...
              {phase === 'confirm' && topic && (
                <>
                  ...
                  <p className="text-sm text-secondary">
                    ...
                    o contexto da área do cliente (demo).
                  </p>
